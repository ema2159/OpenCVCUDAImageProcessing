...
int main(int argc, char **argv) {
  ...
  cv::Mat h_interm(h_img.rows, h_img.cols, CV_8UC3, cv::Scalar(0, 255, 0));
  cv::Mat h_result(h_img.rows, h_img.cols, CV_8UC3, cv::Scalar(0, 255, 0));
  cv::cuda::GpuMat d_img, d_interm, d_result;
  ...
  cv::cuda::copyMakeBorder(h_interm, h_interm, KERNEL_DIV_2, KERNEL_DIV_2,
                           KERNEL_DIV_2, KERNEL_DIV_2, cv::BORDER_REPLICATE);

  d_img.upload(h_img);
  d_interm.upload(h_interm);
  d_result.upload(h_result);

  for (int i = 0; i < iter; i++) {
    // First pass
    startCUDA(d_img, d_interm, KERNEL_SIZE, SIGMA, true);
    // Second pass
    startCUDA(d_interm, d_result, KERNEL_SIZE, SIGMA, false);
  }
