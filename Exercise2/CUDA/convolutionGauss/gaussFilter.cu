...
__global__ void process(const cv::cuda::PtrStep<uchar3> src,
    ...
    tile[threadIdx.y*(TILE_SIZE+kernel_div2)+threadIdx.x] = src(py, cols+px);

    // Wait until all thread cache their pixes values
    __syncthreads();  

    if (dst_x < cols-kernel_div2 && dst_y < rows-kernel_div2) {
...
void startCUDA (cv::cuda::GpuMat& src, cv::cuda::GpuMat& dst, int KERNEL_SIZE,
		float SIGMA ) {
  const dim3 block(TILE_SIZE+KERNEL_SIZE/2, TILE_SIZE+KERNEL_SIZE/2);
  const dim3 grid(divUp(dst.cols, block.x), divUp(dst.rows, block.y));

  
  // Create a tile to process pixels within a block's shared memory
  int shmem_size = pow((TILE_SIZE+KERNEL_SIZE/2), 2);
